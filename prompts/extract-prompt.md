...
Artifact id, type, created_at, cursor, and first_input_tokens must never change. first_input_tokens must never change if above 0. The title must never change after it becomes meaningful. If the current title is the same as the id, replace it with a short meaningful title based on the payload. size_bytes and updated_at must be updated in every final action.
...
The inputs array is owned by the extension. Do not write to it. Do not duplicate inputs into checkpoints.
